...
struct Segment_Tree{
    ...
    node pull(node &A, node &B){
        node C;
        C.ma=max({0LL, A.ma, B.ma, A.suf+B.pre});
        C.sum=A.sum+B.sum;
    ...
    void build(vector<int> &v, int idx=0, int ll=0, int rr=n){
        if (rr-ll==1){
            arr[idx].ma=max(0LL, v[ll]);
            arr[idx].sum=v[ll];
            arr[idx].pre=max(0LL, v[ll]);
            arr[idx].suf=max(0LL, v[ll]);
            return;
        }
    ...
    void update(int pos, int val, int idx=0, int ll=0, int rr=n){
        if (rr-ll==1){
            arr[idx].ma=max(0LL, val);
            arr[idx].sum=val;
            arr[idx].pre=max(0LL, val);
            arr[idx].suf=max(0LL, val);
            return;
        }
