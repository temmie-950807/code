...
int n, q;
int type, x, y;
vector<int> rr(MAX_SIZE), dsu(MAX_SIZE);

// function
int find_rr(int x){
    if (rr[x]==x) return x;
    else return rr[x]=find_rr(rr[x]);
}

int find_dsu(int x){
    if (dsu[x]==x) return x;
    else return dsu[x]=find_dsu(dsu[x]);
}
...
void solve(){
    ...
    for (int i=0 ; i<q ; i++){
        ...
        if (type==2){
            int now=find_rr(x);
            while (now+1<=y){
                rr[now]=find_rr(now+1);               // 更新此點的最右值
                dsu[find_dsu(now)]=find_dsu(rr[now]); // 原本的點被右邊的點合併
                now=rr[now];                          // 把標準變成右邊的點
            }
        }
        if (type==3){
            if (find_dsu(x)==find_dsu(y)) cout << "YES" << endl;
            else cout << "NO" << endl;
        }
    }
    return;
